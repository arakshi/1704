...
def seed_default_users() -> list[User]:
    ...
    default_users_data = [
        ("driver", "123456", UserRole.DRIVER.value, "Водитель"),
        ("accountant", "123456", UserRole.ACCOUNTANT.value, "Бухгалтер"),
        ("admin", "123456", UserRole.ADMIN.value, "Администратор"),
    ]

    changed = False
    for username, password, role, full_name in default_users_data:
...
def seed_sample_documents(upload_folder: str, force: bool = False) -> int:
    ...
    for i in range(1, 11):
        ...
        file_name = f"sample_doc_{i:02d}.jpg"
        file_path = Path(upload_folder) / file_name
        _create_sample_image(file_path, f"ТЕСТ #{i:02d}")
...
def _create_sample_image(path: Path, caption: str) -> None:
    ...
    draw.text((120, 360), "Этот файл создан автоматически для первичного наполнения.", fill=(40, 50, 70))
