...
- начальные документы (10 шт.) с тестовыми сканами в `uploads/`.
...
- 1С (учет): упрощенный интерфейс просмотра принятых документов.
- Админка: пользователи, документы, создание начальных данных, сброс БД, переинициализация.
...
## Админка: начальные данные
Кнопка «Создать начальные данные» в админке создает не только пользователей,
но и тестовые документы со сканами, чтобы реестр бухгалтера сразу был заполнен.
